...
import { buildDeadlineEmail, sendEmail } from "../lib/emailSender.js";
...
type ThresholdType = "deadline_30d" | "deadline_7d" | "deadline_1d";
...
export async function runDeadlineNotifier(): Promise<void> {
  ...
  // Group by student so we can batch per-user work
  type AppRow = {
    id: string | number;
    status: string;
    student_id: string;
    universities: { id: string | number; name: string; closing_date: string } | null;
...
async function notifyStudent(
  ...
  const prefersDeadlineAlerts = (profile.notification_prefs as { deadline_alerts?: unknown } | null)?.deadline_alerts;
  ...
  const sentSet = new Set(
    (sentLogs ?? []).map(
      (l: { notification_type: string; reference_id: string }) =>
        `${l.notification_type}:${l.reference_id}`
    )
  );

  // Group apps by university so we send one email per university per threshold
  type UniGroup = {
    uniId:        string | number;
    uniName:      string;
    closingDate:  string;
    programmeCount: number;
    // Use the worst (most planning-like) status across all programmes for the badge
    worstStatus:  string;
    anyAppId:     string | number;
  };

  const byUni = new Map<string, UniGroup>();
  for (const app of apps) {
    ...
    if ((app.status ?? "planning") === "planning") group.worstStatus = "planning";
  }
  ...
  for (const [, group] of byUni) {
    ...
    for (const { days, type } of THRESHOLDS) {
      ...
      try {
        ...
        console.log(
          `[deadlineNotifier] Sent ${type} to ${profile.email} for ${group.uniName} (${daysLeft}d left)`
        );
      } catch (err) {
        console.error(
          `[deadlineNotifier] Failed to send ${type} to ${profile.email}:`,
          err
        );
      }
    }
