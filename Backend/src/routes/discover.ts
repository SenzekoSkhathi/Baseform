...
discover.get("/programmes", async (ctx) => {
  ...
  let query = supabaseAdmin
    .from("faculties")
    .select(
      `id, name, aps_minimum, scoring_system, native_score_minimum,
       field_of_study, duration_years, qualification_type, nqf_level,
       places_available, additional_requirements,
       universities!inner(id, name, short_name, logo_url, city, province, application_fee, closing_date, website_url)`,
      { count: "exact" }
    )
    .lte("aps_minimum", aps)
