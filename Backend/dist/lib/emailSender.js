...
exports.buildDeadlineEmail = buildDeadlineEmail;
exports.buildGuardianEmail = buildGuardianEmail;
...
function getResend() {
    if (!_resend)
        _resend = new resend_1.Resend(process.env.RESEND_API_KEY ?? "placeholder");
    return _resend;
}
...
// ── Templates ─────────────────────────────────────────────────────────────────
function buildDeadlineEmail(opts) {
    ...
    <div style="background:${urgencyColor}15;border-left:4px solid ${urgencyColor};border-radius:8px;padding:12px 16px;">
      <p style="margin:0;font-size:14px;font-weight:700;color:${urgencyColor};">${urgency} — ${opts.daysLeft === 1
        ? "Submit today or you'll miss your chance."
        : opts.daysLeft <= 7
            ? "Act now — the deadline is almost here."
            : "Start getting your documents ready."}</p>
    </div>
...
function buildGuardianEmail(opts) {
